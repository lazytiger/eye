...
async fn main() -> Result<()> {
    ...
    let openai_provider = create_openai_compatible("openai", "test-key", "gpt-3.5-turbo")?;
    test_provider(&openai_provider).await?;
    
    // Test OpenRouter provider
    println!("\n2. Testing OpenRouter provider:");
    let openrouter_provider = create_openai_compatible("openrouter", "test-key", "openai/gpt-3.5-turbo")?;
    test_provider(&openrouter_provider).await?;
    
    // Test DeepSeek provider
    println!("\n3. Testing DeepSeek provider:");
    let deepseek_provider = create_openai_compatible("deepseek", "test-key", "deepseek-chat")?;
