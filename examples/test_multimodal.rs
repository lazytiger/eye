...
use anyhow::Result;
use eye::provider::{
    AudioFormat, AudioUrl, ChatMessage, ChatRequest, Content, ContentPart, ImageDetail, ImageUrl,
    ModelCapabilities, Provider, Role, VideoFormat, VideoUrl, create_provider,
};
...
async fn test_image_message(provider: &impl Provider) -> Result<()> {
    ...
    let message2 = ChatMessage::new_multimodal(
        Role::User,
        vec![
            ContentPart::Text {
    ...
    println!(
        "  - Is multimodal: {}",
        response.choices[0].message.is_multimodal()
    );
...
async fn test_audio_message(provider: &impl Provider) -> Result<()> {
    ...
    let message2 = ChatMessage::new_multimodal(
        ...
        vec![
            ContentPart::Text {
                text: "What language is this?".to_string(),
            },
...
async fn test_video_message(provider: &impl Provider) -> Result<()> {
    ...
    let message2 = ChatMessage::new_multimodal(
        ...
        vec![
            ContentPart::Text {
                text: "Extract audio from this video:".to_string(),
            },
...
async fn test_mixed_message(provider: &impl Provider) -> Result<()> {
    ...
    let message = ChatMessage::new_multimodal(
        ...
        vec![
            ContentPart::Text {
                text: "Analyze this content:".to_string(),
            },
            ContentPart::ImageUrl {
                image_url: ImageUrl {
...
fn test_capabilities(provider: &impl Provider) {
    ...
    println!(
        "    - TEXT_GENERATION: {}",
        capabilities.contains(ModelCapabilities::TEXT_GENERATION)
    );
    println!(
        "    - FUNCTION_CALLING: {}",
        capabilities.contains(ModelCapabilities::FUNCTION_CALLING)
    );
    println!(
        "    - VISION: {}",
        capabilities.contains(ModelCapabilities::VISION)
    );
    println!(
        "    - AUDIO_INPUT: {}",
        capabilities.contains(ModelCapabilities::AUDIO_INPUT)
    );
    println!(
        "    - VIDEO_INPUT: {}",
        capabilities.contains(ModelCapabilities::VIDEO_INPUT)
    );
    println!(
        "    - OBJECT_GENERATION: {}",
        capabilities.contains(ModelCapabilities::OBJECT_GENERATION)
    );
