...
async fn test_messages() {
    ...
    if response.status().is_success() {
        let messages_response: AnthropicMessagesResponse =
            response.json().await.expect("Failed to parse response");
        println!("Messages response: {:?}", messages_response);
    } else {
        println!("Messages request failed: {}", response.status());
    }
}
...
async fn test_responses() {
    ...
    // This is beta endpoint, might fail or change
    if response.status().is_success() {
        let responses_response: OpenResponsesNonStreamingResponse =
            response.json().await.expect("Failed to parse response");
        println!("Responses response: {:?}", responses_response);
    } else {
        println!("Responses request failed: {}", response.status());
    }
}
